...
async function analyzeFile(filePath: string, format: 'object' | 'text' = 'object'): Promise<any> {
  const fileSize = await GetFileSize(filePath)

  const mi = await MediaInfoFactory({ format, locateFile: () => 'mediainfo.wasm' })
...
// Retourne la sortie mediainfo brute (format 'text', identique au binaire CLI),
// sans aucune retouche locale.
export async function getMediaInfoCLIText(filePath: string): Promise<string> {
  const text = await analyzeFile(filePath, 'text')
